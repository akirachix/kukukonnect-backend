...
class ThresholdAPITests(TestCase):
    ...
    def test_get_device_name(self):
        response = self.client.get(f'/api/device-name/{self.test_device_id}/')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['device_name'], "Test Coop")

    def test_put_device_name(self):
        response = self.client.put(
            f'/api/device-name/{self.test_device_id}/',
            data={"device_name": "New Test Coop"},
            format='json'
        )
        self.assertEqual(response.status_code, 200)
        self.mcu.refresh_from_db()
